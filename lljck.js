...
软件：   立乐家ck
获取ck：  打开小程序即可------
重写：https://clubwx.hm.liby.com.cn/b2cMiniApi/subscribe
...
let userCookie = ($.isNode() ? process.env.lljck : $.getdata('lljck')) || '';  //设定变量名称lljck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://clubwx.hm.liby.com.cn/b2cMiniApi/subscribe
async function GetRewrite() {
    if($request.url.indexOf(`subscribe`) > -1) {
        let token = $request.headers['X-wxb9f68ca2da513bb2-Token'] ? $request.headers['X-wxb9f68ca2da513bb2-Token'] : $request.headers['X-wxb9f68ca2da513bb2-Token']
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'lljck');
                let ckList = userCookie.split('\n')
                $.msg(`立乐家获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'lljck');
            $.msg(`立乐家获取第1个Cookie成功: ${ck}`)
        }
    }
}
