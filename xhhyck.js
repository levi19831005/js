...
软件：   雪花会员ck
获取ck：  打开小程序即可------
重写：https://scrm-kp-event.crb.cn/member-system-api/api/positionRenovationPage
      url script-request-header
      https://raw.githubusercontent.com/levi19831005/js/main/xhhyck.js
主机：*.crb.cn
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env(" 雪花会员ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.xhhyck : $.getdata('xhhyck')) || '';  //设定变量名称xhhyck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://scrm-kp-event.crb.cn/member-system-api/api/positionRenovationPage
async function GetRewrite() {
    if($request.url.indexOf(`positionRenovationPage`) > -1) {
        let token = $request.headers['JWT-TOKEN'] ? $request.headers['JWT-TOKEN'] : $request.headers['JWT-TOKEN']
        let token1 = $request.headers.Cookie ? $request.headers.Cookie : $request.headers.Cookie
        let ck = token1 + '&' + token
        if(!token) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'xhhyck');
                let ckList = userCookie.split('\n')
                $.msg(`雪花会员获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'xhhyck');
            $.msg(`雪花会员获取第1个Cookie成功: ${ck}`)
        }
    }
}
