...
软件：  e览衢江获取ck
获取ck：  打开app即可------
重写：https://vapp.tmuyun.com/api/user_mumber
      url script-request-header 
      https://raw.githubusercontent.com/levi19831005/js/main/elqjck.js
主机：vapp.tmuyun.com
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env("e览衢江获取Ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.qdck : $.getdata('elqjck')) || '';  //设定变量名称qdck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://vapp.tmuyun.com/api/user_mumber
async function GetRewrite() {
    if($request.url.indexOf(`user_mumber`) > -1) {
        let token = $request.headers['X-SESSION-ID'] ? $request.headers['X-SESSION-ID']  : $request.headers['X-SESSION-ID'] 
        let ck = token
        if(!token) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'elqjck');
