...
*/
const $ = new Env(" 深圳湾春茧未来荟获取Ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.szwck : $.getdata('szwck')) || '';  //设定变量名称szwck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://program.springcocoon.com/szbay/api/services/app/banner/GetBannerByNameAsync
async function GetRewrite() {
    if($request.url.indexOf(`GetBannerByNameAsync`) > -1) {
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'szwck');
                let ckList = userCookie.split('\n')
                $.msg(`深圳湾获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'szwck');
            $.msg(`深圳湾获取第1个Cookie成功: ${ck}`)
        }
    }
}
