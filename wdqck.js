...
软件：   我德清
获取ck：  打开app即可------
重写：http://mapi.yingxi.tv/api/v1
      url script-request-header
      https://raw.githubusercontent.com/levi19831005/js/main/wdqck.js
主机：mapi.yingxi.tv
...
let userCookie = ($.isNode() ? process.env.wdqck : $.getdata('wdqck')) || '';  //设定变量名称wdqck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 http://mapi.yingxi.tv/api/v1
async function GetRewrite() {
    if($request.url.indexOf(`api/v1`) > -1) {
        let token = $request.url.split("access_token=")[1].split("&")[0] ? $request.url.split("access_token=")[1].split("&")[0] : $request.url.split("access_token=")[1].split("&")[0]
        let ck = token
        if(!token) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'wdqck');
                let ckList = userCookie.split('\n')
                $.msg(`我德清获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'wdqck');
            $.msg(`我德清获取第1个Cookie成功: ${ck}`)
        }
    }
}
