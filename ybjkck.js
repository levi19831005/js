...
软件：   益宝健康ck
获取ck：  打开小程序即可------
重写：https://app.ai-emma.com/app/system/messageRed
      url script-request-header
      https://raw.githubusercontent.com/levi19831005/js/main/ybjkck.js
主机：app.ai-emma.com
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env(" 益宝健康ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.ybjkck : $.getdata('ybjkck')) || '';  //设定变量名称ybjkck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://app.ai-emma.com/app/system/messageRed
async function GetRewrite() {
    if($request.url.indexOf(`system/messageRed`) > -1) {
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'ybjkck');
                let ckList = userCookie.split('\n')
                $.msg(`益宝健康获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'ybjkck');
            $.msg(`益宝健康获取第1个Cookie成功: ${ck}`)
        }
    }
}
