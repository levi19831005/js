...
3.新增浏览商品获取水滴
本地重写:
act/cdn/cloudConfig.json url script-request-header ttl.js
hostname = miniapp.you.163.com
正常登录后进入小程序首页获取变量，变量名:ttlhd
支持多账号
boxjs地址: https://raw.githubusercontent.com/danchaofan12138/danchaofan/main/ziyong/danchaofan.json

脚本地址: https://raw.githubusercontent.com/danchaofan12138/danchaofan/main/ziyong/wangyi/ttl.js
...
let ttlhdArr = [];
let msg = '';
let ttlhd = $.getdata('ttlhd')
//let num = 3
/////////////////////////////////////////////////////////

let isttlck = typeof $request !== 'undefined'
if (isttlck) {
   ttlck();
   $.done()
};
...
if ($.isNode()) {
  if (process.env.ttlhd&& process.env.ttlhd.indexOf('@') > -1) {
   ttlhd= process.env.ttlhd.split('@');
   console.log(`您选择的是用"@"隔开\n`)
  }
  else if (process.env.ttlhd&& process.env.ttlhd.indexOf('\n') > -1) {
   ttlhd= process.env.ttlhd.split('\n');
...
{
   const ttlhd = JSON.stringify($request.headers.apitoken)
   if(ttlhd) $.setdata(ttlhd,`ttlhd${status}`)
   $.log(`获取ttlhd请求: 成功,ttlhd: ${ttlhd}`)
    $.msg($.name, "", `${status}` + 'ttlhd获取成功！')
}
}

async function ttlqd() {
    let url = {
        url: `https://miniapp.you.163.com/act/money/checkIn/V3/checkIn.json`,
   headers: JSON.parse(ttlhd)
    };
    //console.log(url);
