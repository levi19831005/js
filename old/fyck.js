/*
软件：   福域
获取ck：  打开app-我的-开抓包-点订单
重写：https://evosapi.changanford.cn/user/myInfo
      url script-request-header
      https://raw.githubusercontent.com/levi19831005/js/main/fyck.js
主机：evosapi.changanford.cn
...
let userCookie = ($.isNode() ? process.env.fyck : $.getdata('fyck')) || '';  //设定变量名称fyck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://evosapi.changanford.cn/user/myInfo
async function GetRewrite() {
    if($request.url.indexOf(`user/myInfo`) > -1) {
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'fyck');
                let ckList = userCookie.split('\n')
                $.msg(`福域获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'fyck');
            $.msg(`福域获取第1个Cookie成功: ${ck}`)
        }
    }
}
