...
软件：  新黄河获取ck
获取ck：  打开app-我的------
重写：https://api.jinantimes.com.cn/index
      url script-request-body 
      https://raw.githubusercontent.com/levi19831005/js/main/xhhck.js
主机：api.jinantimes.com.cn
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env("新黄河获取Ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.qdck : $.getdata('xhhck')) || '';  //设定变量名称qdck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://api.jinantimes.com.cn/index
async function GetRewrite() {
    if($request.url.indexOf(`index`) > -1) {
        let token = $request.body.match(/token=(.*)/)[1] ? $request.body.match(/token=(.*)/)[1]  : $request.body.match(/token=(.*)/)[1] 
        let ck = token
        if(!token) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'xhhck');
                let ckList = userCookie.split('\n')
                $.msg(`新黄河获取第${ckList.length}个ck成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'xhhck');
            $.msg(`新黄河获取第1个ck成功: ${ck}`)
