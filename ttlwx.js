...
let status,no;
status = (status = ($.getval("ttlwxstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
...
let isttlwxck = typeof $request !== 'undefined'
if (isttlwxck) {
   ttlwxck();
   $.done()
};
...
if ($.isNode()) {
  if (process.env.ttlwxhd&& process.env.ttlwxhd.indexOf('@') > -1) {
   ttlwxhd= process.env.ttlwxhd.split('@');
   console.log(`您选择的是用"@"隔开\n`)
  }
  else if (process.env.ttlwxhd&& process.env.ttlwxhd.indexOf('\n') > -1) {
   ttlwxhd= process.env.ttlwxhd.split('\n');
...
if (!ttlwxhdArr[0]) {
   ...
   console.log(`------------- 共${ttlwxhdArr.length}个账号----------------\n`)
