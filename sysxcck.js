...
软件：  书亦烧仙草获取ck
获取ck：  打开app即可------
重写：https://scrm-prod.shuyi.org.cn/saas-gateway/api/agg-trade/v1/alipay/student/queryCertification/info
      url script-request-header 
      https://raw.githubusercontent.com/levi19831005/js/main/sysxcck.js
主机：scrm-prod.shuyi.org.cn
...
////////////////////qx获取重写的链接中的关键词 https://scrm-prod.shuyi.org.cn/saas-gateway/api/agg-trade/v1/alipay/student/queryCertification/info
async function GetRewrite() {
    if($request.url.indexOf(`queryCertification/info`) > -1) {
        let token = $request.headers.auth ? $request.headers.auth  : $request.headers.auth 
        let ck = token
        if(!token) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'sysxcck');
                let ckList = userCookie.split('\n')
                $.msg(`书亦烧仙草获取第${ckList.length}个myjk-token成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'sysxcck');
            $.msg(`书亦烧仙草获取第1个ck成功: ${ck}`)
        }
    }
}
