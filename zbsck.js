...
软件：   植白说ck
获取ck：  打开小程序即可------
重写：https://zbs.20171026.com/demo/wx/coupon/getUserCoupon
      url script-request-header
      https://raw.githubusercontent.com/levi19831005/js/main/zbsck.js
主机：zbs.20171026.com
...
let userCookie = ($.isNode() ? process.env.zbsck : $.getdata('zbsck')) || '';  //设定变量名称zbsck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://zbs.20171026.com/demo/wx/coupon/getUserCoupon
async function GetRewrite() {
    if($request.url.indexOf(`coupon/getUserCoupon`) > -1) {
        let token = $request.headers['X-Dts-Token'] ? $request.headers['X-Dts-Token'] : $request.headers['X-Dts-Token']
        let ck = token
        if(!token) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'zbsck');
                let ckList = userCookie.split('\n')
                $.msg(`植白说获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'zbsck');
            $.msg(`植白说获取第1个Cookie成功: ${ck}`)
        }
    }
}
