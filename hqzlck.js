...
软件：   红旗智联
获取ck：  打开app即可------
重写：https://hqapp.faw.cn/fawcshop/auth
      url script-request-header
      https://raw.githubusercontent.com/levi19831005/js/main/hqzlck.js
主机：hqapp.faw.cn
...
let userCookie = ($.isNode() ? process.env.hqzlck : $.getdata('hqzlck')) || '';  //设定变量名称hqzlck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://hqapp.faw.cn/fawcshop/auth
async function GetRewrite() {
    if($request.url.indexOf(`auth`) > -1) {
        ...
        let token1 = $request.headers.aid ? $request.headers.aid : $request.headers.aid
        let ck = token + '&' + token1
        if(!token) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'hqzlck');
                let ckList = userCookie.split('\n')
                $.msg(`红旗智联获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'hqzlck');
            $.msg(`红旗智联获取第1个Cookie成功: ${ck}`)
        }
    }
}
