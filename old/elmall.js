...
软件：  饿了么app获取ck
获取ck：  打开软件吃货豆详情即可------
重写：https://h5.ele.me/crossdomainstorage.html
https://raw.githubusercontent.com/levi19831005/js/main/elmall.js
主机名：h5.ele.me
https://h5.ele.me/crossdomainstorage.html
*/
const $ = new Env("饿了么全部获取Ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.elmallck : $.getdata('elmallck')) || '';  //设定变量名称elmallck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://h5.ele.me/crossdomainstorage.html
async function GetRewrite() {
    if($request.url.indexOf(`crossdomainstorage`) > -1) {
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'elmallck');
                let ckList = userCookie.split('\n')
                $.msg(`饿了么获取第${ckList.length}个ck成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'elmallck');
            $.msg(`饿了么获取第1个ck成功: ${ck}`)
