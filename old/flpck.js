...
软件：   飞利浦健康生活Lab
获取ck：  打开app即可------
重写：https://www.phlab.philips.com.cn/lab/cms/cms/furnish/homePage
      url script-request-header
      https://raw.githubusercontent.com/levi19831005/js/main/flpck.js
主机：www.phlab.philips.com.cn
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env(" 飞利浦健康生活Lab");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.flpck : $.getdata('flpck')) || '';  //设定变量名称flpck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://www.phlab.philips.com.cn/lab/cms/cms/furnish/homePage
    ...
    if($request.url.indexOf(`furnish/homePage`) > -1) {
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'flpck');
                let ckList = userCookie.split('\n')
                $.msg(`飞利浦健康生活获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'flpck');
            $.msg(`飞利浦健康生活获取第1个Cookie成功: ${ck}`)
        }
    }
}
