...
软件：  完美人生获取ck
获取ck：  打开app
重写：https://hms.cignacmb.com/activity/cignaInvestment/getUserTaskList
      url script-request-header 
      https://raw.githubusercontent.com/levi19831005/js/main/wmrsck.js
主机：hms.cignacmb.com
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env("完美人生获取Ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.qdck : $.getdata('wmrsck')) || '';  //设定变量名称qdck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://hms.cignacmb.com/activity/cignaInvestment/getUserTaskList
async function GetRewrite() {
    if($request.url.indexOf(`getUserTaskList`) > -1) {
        let token1 = $request.headers.userId ? $request.headers.userId  : $request.headers.userId 
        let token2 = $request.headers.Authorization ? $request.headers.Authorization  : $request.headers.Authorization 
        let ck = token1 + '&' + token2
        if(!token1) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'wmrsck');
                let ckList = userCookie.split('\n')
                $.msg(`完美人生获取第${ckList.length}个ck成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'wmrsck');
            $.msg(`完美人生获取第1个ck成功: ${ck}`)
        }
    }
}
