...
软件：   热度星客ck
获取ck：  打开小程序即可------
重写：http://m.reduxingke.com/api/user
      url script-request-header
      https://raw.githubusercontent.com/levi19831005/js/main/rdxkck.js
主机：m.reduxingke.com
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env(" 热度星客ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.rdxkck : $.getdata('rdxkck')) || '';  //设定变量名称rdxkck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 http://m.reduxingke.com/api/user
async function GetRewrite() {
    if($request.url.indexOf(`api/user`) > -1) {
        let token = $request.headers['Authori-zation'] ? $request.headers['Authori-zation'] : $request.headers['Authori-zation']
        let ck = token
        if(!token) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'rdxkck');
                let ckList = userCookie.split('\n')
                $.msg(`热度星客获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'rdxkck');
            $.msg(`热度星客获取第1个Cookie成功: ${ck}`)
        }
    }
}
