...
let userCookie = ($.isNode() ? process.env.zmjkck : $.getdata('zmjkck')) || '';  //设定变量名称zmjkck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
    ...
    if($request.url.indexOf(`update/V2`) > -1) {
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'zmjkck');
                let ckList = userCookie.split('\n')
                $.msg(`姿美健康获取第${ckList.length}个myjk-token成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'zmjkck');
            $.msg(`姿美健康获取第1个ck成功: ${ck}`)
        }
    }
}
