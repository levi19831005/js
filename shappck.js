/*
软件：  宝山汇、绿色青浦、美谷奉贤、上海长宁、虹口、黄埔、静安、普陀获取ck
获取ck：  打开即可------
重写：media-basic-port/api/app/sys/billboard
      url script-request-header
      https://raw.githubusercontent.com/levi19831005/js/main/shappck.js
主机：*.shmedia.tech
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env("宝山汇、绿色青浦、美谷奉贤、上海长宁、虹口、黄埔、静安、普陀获取Ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.shappck : $.getdata('shappck')) || '';  //设定变量名称shappck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 media-basic-port/api/app/sys/billboard
async function GetRewrite() {
    if($request.url.indexOf(`billboard`) > -1) {
