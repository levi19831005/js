...
let userCookie = ($.isNode() ? process.env.zxzwnwxck : $.getdata('zxzwnwxck')) || '';  //设定变量名称zxzwnwxck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://zxapi.yili.com/api/order/getCount
    ...
    if($request.url.indexOf(`getCount`) > -1) {
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'zxzwnwxck');
                let ckList = userCookie.split('\n')
                $.msg(`植选植物奶获取第${ckList.length}个ck成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'zxzwnwxck');
            $.msg(`植选植物奶获取第1个ck成功: ${ck}`)
        }
    }
}
