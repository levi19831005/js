...
软件：   中山PLUS
获取ck：  打开app即可------
重写：https://zsrbapp.zsnews.cn/api/user/info
      url script-request-header
      https://raw.githubusercontent.com/levi19831005/js/main/zsplusck.js
主机：zsrbapp.zsnews.cn
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env(" 中山PLUS");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.zsplusck : $.getdata('zsplusck')) || '';  //设定变量名称zsplusck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://zsrbapp.zsnews.cn/api/user/info
async function GetRewrite() {
    if($request.url.indexOf(`user/info`) > -1) {
        let token = $request.headers.sessionToken ? $request.headers.sessionToken : $request.headers.sessionToken
        let ck = token
        if(!token) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'zsplusck');
