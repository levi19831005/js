...
////////////////////qx获取重写的链接中的关键词 https://api1.fjdaily.com/app_if/MobileApp
async function GetRewrite() {
    if($request.url.indexOf(`MobileApp`) > -1) {
        let token = $request.headers.authtoken ? $request.headers.authtoken : $request.headers.authtoken
        if(!token) return;
let ck = token
        if(userCookie) {
