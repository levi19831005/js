...
////////////////////qx获取重写的链接中的关键词 https://miniapp.you.163.com/xhr/front/trade/cart/getMiniCartNum
async function GetRewrite() {
    if($request.url.indexOf(`getMiniCartNum`) > -1) {
        let token = $request.headers['X-WX-3RD-Session'] ? $request.headers['X-WX-3RD-Session']  : $request.headers['X-WX-3RD-Session'] 
        let ck = token
        if(!token) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.msg(`网易严选获取第${ckList.length}ck: ${ck}`)
            }
        } else {
            $.setdata(ck, 'wyyxck');
            $.msg(`网易严选获取第1个ck成功: ${ck}`)
        }
    }
}
