...
软件：  杰士邦小程序获取ck
获取ck：  打开小程序即可------
重写：https://m.jissbon.com/index.php?s=/api/setting/data
...
*/
const $ = new Env("杰士邦获取Ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.jsbck : $.getdata('jsbck')) || '';  //设定变量名称jsbck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://m.jissbon.com/index.php?s=/api/setting/data
async function GetRewrite() {
    if($request.url.indexOf(`api/setting/data`) > -1) {
        let token = $request.headers.Access-Token ? $request.headers.Access-Token : $request.headers.Access-Token
        let ck = token
        if(!token) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'jsbck');
