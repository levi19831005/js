...
软件：  华润通获取ck
获取ck：  打开app即可------
重写：https://mid.huaruntong.cn/api/user/memberinfo/appBootstarp
      url script-response-body 
      hrtck.js
主机：mid.huaruntong.cn
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env("华润通获取Ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.qdck : $.getdata('hrtck')) || '';  //设定变量名称qdck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
async function GetRewrite() {
    if($request.url.indexOf(`appBootstarp`) > -1) {
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'hrtck');
                let ckList = userCookie.split('\n')
                $.msg(`华润通获取第${ckList.length}个token成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'hrtck');
            $.msg(`华润通获取第1个ck成功: ${ck}`)
