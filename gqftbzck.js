...
软件：   广汽丰田bzck
获取ck：  打开小程序即可------
重写：https://gw.nevapp.gtmc.com.cn/time
      url script-request-header
      https://raw.githubusercontent.com/levi19831005/js/main/gqftbzck.js
主机：gw.nevapp.gtmc.com.cn
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env(" 广汽丰田bzck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.gqftbzck : $.getdata('gqftbzck')) || '';  //设定变量名称gqftbzck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://gw.nevapp.gtmc.com.cn/time
async function GetRewrite() {
    if($request.url.indexOf(`time`) > -1) {
        let token = $request.headers.Authorization ? $request.headers.Authorization : $request.headers.Authorization
        let token1 = $request.headers.DeviceId ? $request.headers.DeviceId : $request.headers.DeviceId
        let ck = token + '&' + token1
        if(!token) return;
        if(userCookie) {
