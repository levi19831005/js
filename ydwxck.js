...
软件：  一点万象获取ck
获取ck：  打开app即可------
重写：https://app.mixcapp.com/mixc/gateway
      url script-body
       ydwxck.js
主机：app.mixcapp.com
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env("一点万象获取Ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.qdck : $.getdata('ydwxck')) || '';  //设定变量名称qdck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://app.mixcapp.com/mixc/gateway
    ...
    if($request.url.indexOf(`gateway`) > -1) {
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'ydwxck');
                let ckList = userCookie.split('\n')
                $.msg(`一点万象获取第${ckList.length}个token成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'ydwxck');
            $.msg(`一点万象获取第1个ck成功: ${ck}`)
        }
    }
}
