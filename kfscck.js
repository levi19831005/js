...
软件：   卡夫商城ck（积分通立乐家）
获取ck：  打开小程序即可------
重写：https://mapi-ec.weimob.com/api3/ec/mbp/card/queryUserOpenIdInfo
      url script-request-header
      https://raw.githubusercontent.com/levi19831005/js/main/kfscck.js
主机：mapi-ec.weimob.com
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env(" 卡夫商城ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.kfscck : $.getdata('kfscck')) || '';  //设定变量名称kfscck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://mapi-ec.weimob.com/api3/ec/mbp/card/queryUserOpenIdInfo
async function GetRewrite() {
    if($request.url.indexOf(`queryUserOpenIdInfo`) > -1) {
        let token = $request.headers['X-WX-Token'] ? $request.headers['X-WX-Token'] : $request.headers['X-WX-Token']
        let token1 = $request.headers['weimob-wid'] ? $request.headers['weimob-wid'] : $request.headers['weimob-wid']
        let ck = token + '&' + token1
        if(!token) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'kfscck');
                let ckList = userCookie.split('\n')
                $.msg(`卡夫商城获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'kfscck');
            $.msg(`卡夫商城获取第1个Cookie成功: ${ck}`)
        }
    }
}
