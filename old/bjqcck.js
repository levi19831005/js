...
*/
const $ = new Env(" 北京现代ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.bjqcck : $.getdata('bjqcck')) || '';  //设定变量名称bjqcck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://api.smartservice.bjev.com.cn/gateway-api//v1/notice/user
async function GetRewrite() {
    if($request.url.indexOf(`notice/user`) > -1) {
        let token = $request.url.match(/id=(.*)/)[1] ? $request.url.match(/id=(.*)/)[1] : $request.url.match(/id=(.*)/)[1]
        let token3 = token.replace(/&type=1/g, "").replace(/\s+/g, "")
        ...
        let ck = token3 + '&' + token2
        if(!token) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'bjqcck');
                let ckList = userCookie.split('\n')
                $.msg(`北京现代获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'bjqcck');
            $.msg(`北京现代获取第1个Cookie成功: ${ck}`)
        }
    }
}
