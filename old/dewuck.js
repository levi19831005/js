...
////////////////////qx获取重写的链接中的关键词 https://app.dewu.com/hacking-tree/v1/user/init
async function GetRewrite() {
    if($request.url.indexOf(`user/init`) > -1) {
        let token1 = $request.headers.SK ? $request.headers.SK  : $request.headers.SK
        let token2 = $request.headers.shumeiid ? $request.headers.shumeiid  : $request.headers.shumeiid
        let token3 = $request.headers['x-Auth-Token'] ? $request.headers['x-Auth-Token']  : $request.headers['x-Auth-Token'] 
        let token4 = $request.headers.uuid ? $request.headers.uuid  : $request.headers.uuid  
        let token5 = $request.headers.duToken ? $request.headers.duToken  : $request.headers.duToken  
        let token = token1 + '&' + token2 + '&' + token3 + '&' + token4 + '&' + token5
        let ck = token
        if(!token) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'dewuck');
                let ckList = userCookie.split('\n')
                $.msg(`得物获取第${ckList.length}ck: ${ck}`)
            }
        } else {
            $.setdata(ck, 'dewuck');
            $.msg(`得物获取第1个ck成功: ${ck}`)
