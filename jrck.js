...
软件：   洁柔官方自营旗舰店wx小程序
获取ck：  打开app即可------
重写：https://xapi.weimob.com/fe/mapi/user/loginX
      url script-response-body
      https://raw.githubusercontent.com/levi19831005/js/main/jrck.js
主机：xapi.weimob.com
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env(" 洁柔官方自营旗舰店wx小程序");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.jrck : $.getdata('jrck')) || '';  //设定变量名称jrck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
async function GetRewrite() {
    if($request.url.indexOf(`loginX`) > -1) {
        ...
        let ck = token + '&' + token1 + '&' + token2
        if(!token) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'jrck');
                let ckList = userCookie.split('\n')
                $.msg(`洁柔获取第${ckList.length}个CK成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'jrck');
            $.msg(`洁柔获取第1个CK成功: ${ck}`)
