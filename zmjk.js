 ...
 * ========= 青龙--配置文件 =========
 * 姿美健康app-右下角我的-中间姿美麦田 7.24完成
 * 进去完成新手任务后，点赞一次抓包，抓jk-agw-m.simeitol.com链接下myjk-token数据，ey开头的那个，其他任务不要手动做，不然脚本会报错。
...
async function zzxx() {
 ...
 async function tzxx() {
	let options = {
		method: 'GET',
		url: `${hostname}/api/health/myjkUserdataUserWeightRecord/getNewestWeightRecord`,
		headers: {
			'Host': host,
			'content-type': 'application/json',
			'myjk-ct' : `ios`,
	...
	let result = await httpRequest(options, `当前体重`);

	if (result.code == 0) {
		console.log(`\n 当前体重🐘: ${result.data.weight}`);
		msg += `\n 当前体重🐘: ${result.data.weight}`;
		jrtz = result.data.weight;//获取当前体重数据
	}
} 
 ...
 async function plyc() {
	 let options = {
		 ...
		 body: `{"contentId":"${sj}","authorUserid":"1541335765363326976","mediaType":"text","content":"点赞","channelType":"IOS","referenceUserId":"1541335765363326976","type":0}`
	 };
	 let result = await httpRequest(options, `记录列表`);
