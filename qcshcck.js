...
软件：  屈臣氏撸猫合成游戏获取ck
获取ck：  打开app-我的------
重写：https://watsonscat.watsonsvip.com.cn/prd/api/configs
      url script-request-header 
      https://raw.githubusercontent.com/levi19831005/js/main/qcshcck.js
主机：watsonscat.watsonsvip.com.cn
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env("屈臣氏撸猫合成游戏获取Ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.qdck : $.getdata('qcshcck')) || '';  //设定变量名称qdck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
    ...
    if($request.url.indexOf(`api/configs`) > -1) {
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'qcshcck');
                let ckList = userCookie.split('\n')
                $.msg(`屈臣氏撸猫合成游戏获取第${ckList.length}个token成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'qcshcck');
            $.msg(`屈臣氏撸猫合成游戏获取第1个ck成功: ${ck}`)
        }
    }
}
