...
*/
const $ = new Env(" 掌上金东ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.zsjdck : $.getdata('zsjdck')) || '';  //设定变量名称zsjdck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
    ...
    if($request.url.indexOf(`c/channels`) > -1) {
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'zsjdck');
                let ckList = userCookie.split('\n')
                $.msg(`掌上金东获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'zsjdck');
            $.msg(`掌上金东获取第1个Cookie成功: ${ck}`)
        }
    }
}
