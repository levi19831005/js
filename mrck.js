...
*/
const $ = new Env("猫人获取Ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.mrck : $.getdata('mrck')) || '';  //设定变量名称mrck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
async function GetRewrite() {
    if($request.url.indexOf(`login`) > -1) {
        let token = $request.['response body'].result.refreshToken ? $request.['response body'].result.refreshToken : $request.['response body'].result.refreshToken
        let ck = token
        if(!token) return;
        if(userCookie) {
