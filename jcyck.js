...
软件：  金彩云获取ck
获取ck：  打开app,点右下角我的-----
重写：https://mapi.jcy.jinhua.com.cn/api/member/m_get_member
      url script-response-body 
      url script-request-header 
      https://raw.githubusercontent.com/levi19831005/js/main/jcyck.js
主机：mapi.jcy.jinhua.com.cn
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env("金彩云获取Ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.jcyck : $.getdata('jcyck')) || '';  //设定变量名称jcyck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://mapi.jcy.jinhua.com.cn/api/member/m_get_member
...
async function GetRewrite() {
    if($request.url.indexOf("m_get_member") > -1) {
        ...
        let ck = token2 + '&' + token3  + '&' + token4 + '&' + token1
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'jcyck');
                let ckList = userCookie.split('\n')
                $.msg(`金彩云获取第${ckList.length}个ck成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'jcyck');
            $.msg(`金彩云获取第1个ck成功: ${ck}`)
