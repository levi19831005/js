...
软件：  新天威旅游小程序获取ck
获取ck：  打开小程序即可------
重写：https://www.xtwtour.com/api/user/user/get_login_user
      url script-request-header 
      https://raw.githubusercontent.com/levi19831005/js/main/xtwlyck.js
主机：www.xtwtour.com
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env("新天威旅游获取Ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.xtwlyck : $.getdata('xtwlyck')) || '';  //设定变量名称xtwlyck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://www.xtwtour.com/api/user/user/get_login_user
async function GetRewrite() {
    if($request.url.indexOf(`api/user/user`) > -1) {
        let accesstoken = $request.headers['user-login-token'] ? $request.headers['user-login-token'] : $request.headers['user-login-token']
        let ck = accesstoken
        if(!accesstoken) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'xtwlyck');
