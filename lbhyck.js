...
软件：   立白会员俱乐部ck（积分通立乐家）
获取ck：  打开小程序即可------
重写：https://clubwx.hm.liby.com.cn/b2cMiniApi/coupon
      url script-request-header
      https://raw.githubusercontent.com/levi19831005/js/main/lbhyck.js
主机：clubwx.hm.liby.com.cn
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env(" 立白会员俱乐部ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.lbhyck : $.getdata('lbhyck')) || '';  //设定变量名称lbhyck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://clubwx.hm.liby.com.cn/b2cMiniApi/coupon
async function GetRewrite() {
    if($request.url.indexOf(`b2cMiniApi/coupon`) > -1) {
        let token = $request.headers['X-wxc503ab22be5af334-Token'] ? $request.headers['X-wxc503ab22be5af334-Token'] : $request.headers['X-wxc503ab22be5af334-Token']
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'lbhyck');
                let ckList = userCookie.split('\n')
                $.msg(`立白会员俱乐部获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'lbhyck');
            $.msg(`立白会员俱乐部获取第1个Cookie成功: ${ck}`)
        }
    }
}
