...
软件：  伊利牛奶小程序
获取ck：  打开小程序即可------
重写：https://msmarket.msx.digitalyili.com/gateway
      url script-request-header 
      https://raw.githubusercontent.com/levi19831005/js/main/yilick.js
主机：msmarket.msx.digitalyili.com
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env("伊利牛奶小程序获取Ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.yilick : $.getdata('yilick')) || '';  //设定变量名称yilick
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://msmarket.msx.digitalyili.com/gateway
    ...
    if($request.url.indexOf(`gateway`) > -1) {
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'yilick');
                let ckList = userCookie.split('\n')
                $.msg(`伊利牛奶获取第${ckList.length}个ck成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'yilick');
            $.msg(`伊利牛奶获取第1个ck成功: ${ck}`)
        }
    }
}
