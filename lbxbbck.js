...
*/
const $ = new Env(" 立白小白白ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.lbxbbck : $.getdata('lbxbbck')) || '';  //设定变量名称lbxbbck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
    ...
    if($request.url.indexOf(`miniprogram/mini`) > -1) {
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'lbxbbck');
                let ckList = userCookie.split('\n')
                $.msg(`立白小白白获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'lbxbbck');
            $.msg(`立白小白白获取第1个Cookie成功: ${ck}`)
        }
    }
}
