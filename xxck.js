...
软件：   心喜
获取ck：  打开小程序即可------
重写：https://cdn-api.xinc818.com/mini/index/v3
      url script-request-header
      https://raw.githubusercontent.com/levi19831005/js/main/xxck.js
主机：cdn-api.xinc818.com
...
let userCookie = ($.isNode() ? process.env.xxck : $.getdata('xxck')) || '';  //设定变量名称xxck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://cdn-api.xinc818.com/mini/index/v3
async function GetRewrite() {
    if($request.url.indexOf(`index/v3`) > -1) {
        let token = $request.headers.sso ? $request.headers.sso : $request.headers.sso
        let ck = token
        if(!token) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'xxck');
                let ckList = userCookie.split('\n')
                $.msg(`心喜获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'xxck');
            $.msg(`心喜获取第1个Cookie成功: ${ck}`)
        }
    }
}
