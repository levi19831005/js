...
*/
const $ = new Env(" 立白好爸爸ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.lbhbbck : $.getdata('lbhbbck')) || '';  //设定变量名称lbhbbck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
    ...
    if($request.url.indexOf(`appEventRecord`) > -1) {
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'lbhbbck');
                let ckList = userCookie.split('\n')
                $.msg(`立白好爸爸获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'lbhbbck');
            $.msg(`立白好爸爸获取第1个Cookie成功: ${ck}`)
        }
    }
}
