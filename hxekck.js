...
软件：  鸿星尔克小程序获取ck
获取ck：  打开小程序签到即可------
重写：https://hope.demogic.com/gic-wx-app/member_sign.json
      url script-request-body 
      https://raw.githubusercontent.com/levi19831005/js/main/hxekck.js
主机：hope.demogic.com
boxjs订阅：https://raw.githubusercontent.com/levi19831005/js/main/levi19831005.boxjs.json
*/
const $ = new Env("鸿星尔克获取Ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.hxekck : $.getdata('hxekck')) || '';  //设定变量名称hxekck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://hope.demogic.com/gic-wx-app/member_sign.json
async function GetRewrite() {
    if($request.url.indexOf(`member_sign`) > -1) {
        let accesstoken = $request.body? $request.body : $request.body
        let ck = accesstoken
        if(!accesstoken) return;
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '\n' + ck
                $.setdata(userCookie, 'hxekck');
                let ckList = userCookie.split('\n')
                $.msg(`鸿星尔克获取第${ckList.length}个ck成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'hxekck');
            $.msg(`鸿星尔克获取第1个ck成功: ${ck}`)
        }
    }
}
