...
软件：   长拉拉ck
获取ck：  打开app即可------
重写：https://changlala.zbyjkj.com/api/wanlshop/token/check
      url script-request-header
      https://raw.githubusercontent.com/levi19831005/js/main/cllck.js
主机：changlala.zbyjkj.com
...
let userCookie = ($.isNode() ? process.env.cllck : $.getdata('cllck')) || '';  //设定变量名称cllck
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://changlala.zbyjkj.com/api/wanlshop/token/check
async function GetRewrite() {
    if($request.url.indexOf(`token/check`) > -1) {
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'cllck');
                let ckList = userCookie.split('\n')
                $.msg(`长拉拉获取第${ckList.length}个Cookie成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'cllck');
            $.msg(`长拉拉获取第1个Cookie成功: ${ck}`)
        }
    }
}
