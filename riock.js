...
软件：  锐澳获取ck
获取ck：  打开软件即可------
重写：https://club.rioalc.com/api/miniprogram/points-goods/recommended url script-request-header https://raw.githubusercontent.com/levi19831005/js/main/riock.js
主机名：club.rioalc.com
*/
const $ = new Env("锐澳获取Ck");

let envSplitor = ['\n']  //多账号隔开方式
let httpResult, httpReq, httpResp                                                                   ////这个不懂
let userCookie = ($.isNode() ? process.env.riock : $.getdata('riock')) || '';  //设定变量名称riock
let userList = []                                                                                   ////这个不懂
let userIdx = 0                                                                                     ////这个不懂
...
////////////////////qx获取重写的链接中的关键词 https://club.rioalc.com/api/miniprogram/points-goods/recommended
async function GetRewrite() {
    if($request.url.indexOf(`recommended`) > -1) {
        ...
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                ...
                $.setdata(userCookie, 'riock');
                let ckList = userCookie.split('\n')
                $.msg(`锐澳获取第${ckList.length}个ck成功: ${ck}`)
            }
        } else {
            $.setdata(ck, 'riock');
            $.msg(`锐澳获取第1个ck成功: ${ck}`)
